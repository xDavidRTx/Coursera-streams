...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    ...
    val legal = b.legalNeighbors
     val list=
       for{
        x <- legal
      } yield (x._1,x._2 ::history)
      list.toStream
    }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      ...
      case (p, moves)#::rest => {
        rest ++ from(rest ++ newNeighborsOnly(neighborsWithHistory(p,moves), explored + p), explored + p)
      }
    }
  }
